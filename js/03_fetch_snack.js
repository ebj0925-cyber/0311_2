fetch("./json/03_snack.json")
  .then((response) => {
    if (!response.ok) {
      throw new Error("야식 JSON 파일을 불러오지 못했습니다.");
    }
    return response.json();
  })
  .then((data) => {
    const list = document.querySelector(".snack_list");
    let html = "";
  ...
  .catch((error) => {
    console.error(error);
    document.querySelector(".snack_list").innerHTML =
      "<p>야식 상품 데이터를 불러오지 못했습니다.</p>";
  });