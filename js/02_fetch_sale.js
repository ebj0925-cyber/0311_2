fetch("./json/02_sale.json")
  .then((response) => {
    if (!response.ok) {
      throw new Error("세일 JSON 파일을 불러오지 못했습니다.");
    }
    return response.json();
  })
  .then((data) => {
    const list = document.querySelector(".sale_list");
    let html = "";
        ...
        <li class="product_item">
          <a href="#">
            ...
            <div class="product_info">
              ...
              ${item.tag ? `<span class="item_tag">${item.tag}</span>` : ""}
            </div>
          </a>
        </li>
      `;
  ...
  .catch((error) => {
    console.error(error);
    document.querySelector(".sale_list").innerHTML =
      "<p>세일 상품 데이터를 불러오지 못했습니다.</p>";
  });